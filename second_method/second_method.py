...
def denoising(noisy_image, dict_size, sparsity):
    ...
    print(train_noisy_patches)
    print(len(train_noisy_patches))
    print(len(train_noisy_patches[0]))
    print(dict_size)
    print(sparsity)
...
noisy_image = cv2.imread(args['image2'], 0)
...
cv2.imshow('Original vs Reconstructed', numpy_horizontal_concat)
